...
pub mod ierc721a {
    ...
    impl<M> std::fmt::Debug for IERC721A<M> {
    ...
    impl ethers::core::abi::AbiDecode for IERC721ACalls {
        fn decode(
            ...
            {
                return Ok(IERC721ACalls::SafeTransferFromWithFromAndToAndData(decoded));
            }
            if let Ok(decoded) =
    ...
    impl ethers::core::abi::AbiEncode for IERC721ACalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ...
                IERC721ACalls::SafeTransferFromWithFromAndToAndData(element) => element.encode(),
    ...
    impl ::std::fmt::Display for IERC721ACalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                ...
                IERC721ACalls::SafeTransferFromWithFromAndToAndData(element) => element.fmt(f),
    ...
    impl ::std::convert::From<SafeTransferFromWithFromAndToAndDataCall> for IERC721ACalls {
        fn from(var: SafeTransferFromWithFromAndToAndDataCall) -> Self {
            IERC721ACalls::SafeTransferFromWithFromAndToAndData(var)
        }
    }
