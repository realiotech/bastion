...
error MarketPlaceCannotSetAddressZero();
// error NoTilesSelected();
// error RegionAlreadyOwned();
error MarketPlaceNotAuthorized();
...
contract Marketplace is ReentrancyGuard {
    ...
    */
    ...
    constructor(address _landNFT) {
        if (_landNFT == address(0)) {
            revert MarketPlaceCannotSetAddressZero();
        }
    ...
    function setLandBank(address _landBank) external {
        if (msg.sender != admin) {
            revert MarketPlaceNotAuthorized();
        }
        if (_landBank == address(0)) {
            revert MarketPlaceCannotSetAddressZero();
        }
        landBank = _landBank;
    }
